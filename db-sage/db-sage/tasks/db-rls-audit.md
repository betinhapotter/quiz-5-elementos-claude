...
./squads/super-agentes/scripts/database-operations/rls-test-suite.sh {table_name}

# Test all tables with RLS
for table in $(psql "$SUPABASE_DB_URL" -t -A -c "SELECT tablename FROM pg_tables WHERE schemaname='public' AND rowsecurity=true"); do
  ./squads/super-agentes/scripts/database-operations/rls-test-suite.sh $table
done
