...
## 🚀 NEW: Use Automated Query Killer (RECOMMENDED)

**Token Savings: 85% | Time Savings: ~80%**

```bash
# Execute SQL file with transaction safety
./squads/super-agentes/scripts/database-operations/query-killer.sh --file {sql_file}

# Execute inline SQL
./squads/super-agentes/scripts/database-operations/query-killer.sh --sql "{sql_statement}"

# Execute with timeout (automatically cancel if exceeds)
./squads/super-agentes/scripts/database-operations/query-killer.sh --file {sql_file} --timeout 30s

# Execute with dry-run (rollback after execution)
./squads/super-agentes/scripts/database-operations/query-killer.sh --file {sql_file} --dry-run
