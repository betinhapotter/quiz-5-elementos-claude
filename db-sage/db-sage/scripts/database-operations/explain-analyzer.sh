...
EXPLAIN_RESULT=$(psql "$SUPABASE_DB_URL" -t -A << SQL 2>/dev/null
${EXPLAIN_CMD}${QUERY}
SQL
)
...
if [ -n "$TABLES" ]; then
    for TABLE in $TABLES; do
        ...
        INDEX_COUNT=$(psql "$SUPABASE_DB_URL" -t -A -c "SELECT COUNT(*) FROM pg_indexes WHERE tablename = '$TABLE';" 2>/dev/null || echo "0")
...
if [ "$FORMAT" = "json" ]; then
    ...
    psql "$SUPABASE_DB_URL" << SQL 2>/dev/null | head -30
