...
```bash
# From project root
bats squads/super-agentes/tests/unit/*.bats
bats squads/super-agentes/tests/integration/*.bats
```

### Run Specific Test File

```bash
bats squads/super-agentes/tests/unit/test_sync_file.bats
```

### Run with TAP Output

```bash
bats --tap squads/super-agentes/tests/unit/*.bats
```
...
@test "sync creates expected files" {
    run ./squads/super-agentes/scripts/sync-to-ides.sh --dry-run
    [ "$status" -eq 0 ]
...
jobs:
  test:
    ...
    steps:
      ...
      - name: Run tests
        run: bats squads/super-agentes/tests/**/*.bats
```
