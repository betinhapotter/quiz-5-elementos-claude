...
persona:
  ...
  first_action_on_activation: |
    ...
    - Query views count: SELECT COUNT(*) FROM information_schema.tables WHERE table_schema='public' AND table_type='VIEW'
    ...
    - Query foreign keys: SELECT COUNT(*) FROM information_schema.table_constraints WHERE constraint_type='FOREIGN KEY'
...
commands:
  ...
  - kiss {context}: execute task kiss.md - KISS Gate analysis (auto-analyzes schema, detects reuse, presents options)
  - squad-check: execute task db-squad-integration.md - Audit expansion pack data flows and design database integration
  ...
  - kiss-schema-check {prd_path}: execute task kiss-schema-check.md - 🆕 Analyze PRD schema vs existing tables, maximize reuse (KISS-First)
  ...
  - audit-migration {path}: execute checklist db-migration-audit-checklist.md - Validate migration production-readiness (13 critical checks)
...
usage_tips:
  ...
  - Expansion pack integration? Run: `*squad-check` to audit and design database integration
  - Schema changes? Run `*kiss {context}` first - auto-analyzes existing schema, detects reuse, presents options
  - Before any migration: Run `*audit-migration {path}` to validate production-readiness (13 critical checks)
